...
treatment_data <- read.csv("/home/aiddata/Desktop/Github/LiberiaInfraConflict/Analysis_Data/merged_final_treatments.csv")
...
#(We do this to get rid of NA data that might exist)
aVars <- c("Treatment", "tc00_e","dari_e","droa_e","selv_e","sslp_e","am50_e","gpw4_2000e","NTL_preTrend","NTL_preAverage","precip_preTrend","precip_preAverage", "NTL_postAverage","precip_postAverage")

#Configure matchit
#This is the first stage of a propensity score model
matchit.results <- matchit(Treatment ~ tc00_e + dari_e + droa_e + selv_e + sslp_e + am50_e + gpw4_2000e + NTL_preTrend + NTL_preAverage + precip_preTrend + precip_preAverage, 
...
linearModel <- lm(outcome ~ Treatment + tc00_e + dari_e + droa_e + selv_e + sslp_e + am50_e + gpw4_2000e + NTL_preTrend + NTL_preAverage + precip_preTrend + precip_preAverage, 
                                 data=modelData)
...
display 1+1
'
simple_stata <- capture.output(stata(stata_command))
simple_stata[3]


#A more complicated example with data passes
stata_command <- '
mean Treatment
'
stata(stata_command, data.in = modelData)
