setwd("/home/aiddata/Desktop/Github/LiberiaInfraConflict")
library(rgdal)
library(ltm)
library(maptools)
library(rgeos)
...
dhs2007.shpfile.location <-"/home/aiddata/Desktop/Github/LiberiaInfraConflict/2007 DHS Liberia/lbge52fl Geocoding/LBGE52FL.shp"
...
concessions.shp.location <-"/home/aiddata/Desktop/Github/LiberiaInfraConflict/confinal/confinal.shp"
...
liberia_data.location <-"/home/aiddata/Desktop/Github/LiberiaInfraConflict/liberia_data.csv"
...
dhs.2007.individual.location <- "/home/aiddata/Desktop/Github/LiberiaInfraConflict/2007 DHS Liberia/lbir51dt Individual/LBIR51FL.DTA"
...
stargazer(LM1,LM4,
          ...
          covariate.labels = c("Treatment","Night-time Lights, Pre-Trend (1992-2006)",
                               ...
                               "Education Level (Secondary)","Religion","Living in Household",
                               "Gender of Head of Household (Female)",
                               "Age of Head of Household",
                               "Wealth (Poorer)","Wealth (Middle)","Wealth (Richer)","Wealth (Richest)",
                               "Wealth (Factor Score)","Marital Status (Married)","Marital Status (Living Together)",
                               "Residing with Partner","Working (Yes)","Occupation (44)","Occupation (62)","Occupation (65)",
                               "DHS Region (North Central)","DHS Region (North Western)",
                               "DHS Region (South Central)","DHS Region (South Eastern A)","DHS Region (South Eastern B)"),
...
stargazer(LM2,LM3,
          ...
          covariate.labels = c("Treatment,","Night-time Lights, Pre-Trend (1992-2006)",
                               ...
                               "Wealth (Poorer)","Wealth (Middle)","Wealth (Richer)","Wealth (Richest)",
                               "Wealth (Factor Score)","Marital Status (Married)","Marital Status (Living Together)",
                               "Residing with Partner","Working (Yes)","Occupation (44)",
                               "Occupation (62)","Occupation (65)",
...
#run cluster.vcov for all models
#first model 
DTA_overlaps@data$outcome <- DTA_overlaps@data$postAvgNTL - DTA_overlaps@data$preAvgNTL
aVars <- c("outcome","Treatment","preTrendNTL","preAvgNTL","postAvgNTL","Edu_Lvl","Religion",
           "Household","Gender","Age","Literacy","Wealth","Marital_Status",
           "Residence","Working","Occupation","DHSREGNA")
LM9 <- lm(outcome ~ Treatment + preTrendNTL + preAvgNTL + postAvgNTL + Edu_Lvl + Religion +
            Household + Gender + Age + Literacy + Wealth +
            Marital_Status + Residence + Working + Occupation + DHSREGNA, 
          data=DTA_overlaps@data[aVars])
cluster1 <- cluster.vcov(LM9,DTA_overlaps@data$DHSREGNA,force_posdef = TRUE)
CMREG1 <- coeftest(LM9,cluster1)
CMREG1
#second model
DTA_overlaps_ext@data$outcome <- DTA_overlaps_ext@data$postAvgNTL - DTA_overlaps_ext@data$preAvgNTL
aVars <- c("outcome","Treatment","preTrendNTL","preAvgNTL","postAvgNTL","Edu_Lvl","Religion",
           "Household","Gender","Age","Literacy","Wealth","Marital_Status",
           "Residence","Working","Occupation","DHSREGNA")
LM10 <- lm(outcome ~ Treatment + preTrendNTL + preAvgNTL + postAvgNTL + Edu_Lvl + Religion +
             Household + Gender + Age + Literacy + Wealth +
             Marital_Status + Residence + Working + Occupation + DHSREGNA, 
           data=DTA_overlaps_ext@data[aVars])
cluster2 <- cluster.vcov(LM10,DTA_overlaps@data$DHSREGNA,force_posdef = TRUE)
CMREG2 <- coeftest(LM10,cluster2)
CMREG2
#third model 
DTA_dist@data$outcome <- DTA_dist@data$postAvgNTL - DTA_dist@data$preAvgNTL
aVars <- c("outcome","Treatment","preTrendNTL","preAvgNTL","postAvgNTL","Edu_Lvl","Religion",
           "Household","Gender","Age","Literacy","Wealth","Marital_Status",
           "Residence","Working","Occupation","DHSREGNA")
LM11 <- lm(outcome ~ Treatment + preTrendNTL + preAvgNTL + postAvgNTL + Edu_Lvl + Religion +
             Household + Gender + Age + Literacy + Wealth +
             Marital_Status + Residence + Working + Occupation + DHSREGNA, 
           data=DTA_dist@data[aVars])
cluster3 <- cluster.vcov(LM11,DTA_dist@data$DHSREGNA,force_posdef = TRUE)
CMREG3 <- coeftest(LM11,cluster3)
CMREG3
#fourth model
DTA_dist_ext@data$outcome <- DTA_dist_ext@data$postAvgNTL - DTA_dist_ext@data$preAvgNTL
aVars <- c("outcome","Treatment","preTrendNTL","preAvgNTL","postAvgNTL","Edu_Lvl","Religion",
           "Household","Gender","Age","Literacy","Wealth","Marital_Status",
           "Residence","Working","Occupation","DHSREGNA")
LM12 <- lm(outcome ~ Treatment + preTrendNTL + preAvgNTL + postAvgNTL + Edu_Lvl + Religion +
             Household + Gender + Age + Literacy + Wealth +
             Marital_Status + Residence + Working + Occupation + DHSREGNA, 
           data=DTA_dist_ext@data[aVars])
cluster4 <- cluster.vcov(LM12,DTA_dist@data$DHSREGNA,force_posdef = TRUE)
CMREG4 <- coeftest(LM12,cluster4)
CMREG4
...
#import liberia boundary file to do overlaps between concessions and boundaries -- used to calculate
#concessions that fall within particular ADMs
lib.bound.loc <- "/home/aiddata/Desktop/Github/LiberiaInfraConflict/LiberiaBoundaries/LBR_adm1.shp"
lib.bound <- readOGR(lib.bound.loc, "LBR_adm1")
lib.bound <- spTransform(lib.bound,CRS("+proj=merc +zone=29 +datum=WGS84"))
lib.bound.Sinoe <- lib.bound[which(lib.bound@data$NAME_1=="Sinoe"),]
lib.bound.Rivercess <- lib.bound[which(lib.bound@data$NAME_1=="River Cess"),]
lib.bound.Bomi <- lib.bound[which(lib.bound@data$NAME_1=="Bomi"),]
lib.bound.Nimba <- lib.bound[which(lib.bound@data$NAME_1=="Nimba"),]
lib.bound.subset <- rbind(lib.bound.Sinoe,lib.bound.Bomi)
lib.bound.subset <- rbind(lib.bound.subset,lib.bound.Nimba)
lib.bound.subset <- rbind(lib.bound.subset,lib.bound.Rivercess)
library(xlsx)
for (i in 1:length(concessions)) 
{
  within_concessions <- over(concessions[i,],lib.bound.subset)
  if(is.na(within_concessions$ID_0))
  {
    min_dist_2 <-min(spDists(concessions[i,],lib.bound.subset,longlat = FALSE))
    concessions@data$subset[i] <- min_dist_2
  }
  else
  {
    concessions@data$subset[i] <- 0
  }
}
#first model with Os removed for preAvgNTL
DTA_dist_0s <- dhs.2007
DTA_dist_0s <- DTA_dist_0s[which(DTA_dist_0s@data$preAvgNTL!=0),]
DTA_dist_0s@data$Treatment <- 0
#Calculate the Median Overlap
med_dist_0s <- median(DTA_dist_0s@data$dist_nearest)
#Set units with overlap > med_overlap to 1:
DTA_dist_0s@data$Treatment <- (as.numeric((DTA_dist_0s@data$dist_nearest) < med_dist_0s))
#Double check:
summary(DTA_dist_0s@data$Treatment)
#Check how many units of observation this leaves us with (our initial n for the
#analysis).
length(DTA_dist_0s@data)
#first stage of psm model for overlap counts, set caliper to .5 
aVars <- c("Treatment","preTrendNTL","preAvgNTL","postAvgNTL","Edu_Lvl","Religion",
           "Household","Gender","Age","Literacy","Wealth","Marital_Status",
           "Residence","Working","Occupation","DHSREGNA")
matchit.dist.0s <- matchit(Treatment ~ preTrendNTL + preAvgNTL + postAvgNTL + Edu_Lvl + Religion +
                             Household + Gender + Age + Literacy + Wealth +
                             Marital_Status + Residence + Working + Occupation + DHSREGNA, 
                           data=DTA_dist_0s@data[aVars],
                           method="nearest",distance="logit", 
                           caliper=0.5)
summary(matchit.dist.0s)
modelDistance_0s <- match.data(matchit.dist.0s)
modelDistance_0s$outcome <- modelDistance_0s$postAvgNTL - modelDistance_0s$preAvgNTL
summary(LM5 <- lm(outcome ~ Treatment + preTrendNTL + preAvgNTL + Edu_Lvl + 
                    Household + Gender + Age + Literacy + Wealth + 
                    Marital_Status + Working + Occupation + 
                    factor(DHSREGNA),
                  data=modelDistance_0s))

summary(LMTEST <- lm(outcome ~ Treatment + preTrendNTL + preAvgNTL,
                  data=modelDistance_0s))

#second linear model with 0s removed for preAvgNTL
#create a new data frame and set all units in the treatment to 0:
DTA_dist_ext_0s <- dhs.2007
DTA_dist_ext_0s <- DTA_dist_ext_0s[which(DTA_dist_ext_0s@data$preAvgNTL!=0.000000e+00),]
DTA_dist_ext_0s@data$Treatment <- 0
#Calculate the Median Nearest Distance
med_dist_ext_0s <- median(DTA_dist_ext_0s$dist_nearest_extractonly)
#Set units with nearest distance < med_dist to 1:
DTA_dist_ext_0s@data$Treatment <- (as.numeric((DTA_dist_ext_0s@data$dist_nearest_extractonly) < 
                                                med_dist_ext_0s))
#Double check:
summary(DTA_dist_ext_0s@data$Treatment)
#Check how many units of observation this leaves us with (our initial n for the
#analysis).
length(DTA_dist_ext_0s@data)
#first stage of psm model for nearest distance
aVars <- c("Treatment","preTrendNTL","preAvgNTL","postAvgNTL","Edu_Lvl","Religion",
           "Household","Gender","Age","Literacy","Wealth","Marital_Status",
           "Residence","Working","Occupation","DHSREGNA")
matchit.dist.ext.0s <- matchit(Treatment ~ preTrendNTL + preAvgNTL + postAvgNTL + Edu_Lvl + Religion +
                                 Household + Gender + Age + Literacy + Wealth +
                                 Marital_Status + Residence + Working + Occupation + DHSREGNA, 
                               data=DTA_dist_ext_0s@data[aVars],
                               method="nearest",distance="logit", 
                               caliper=0.5)
summary(matchit.dist.ext.0s)
modelDistance_ext_0s <- match.data(matchit.dist.ext.0s)
modelDistance_ext_0s$outcome <- modelDistance_ext_0s$postAvgNTL - modelDistance_ext_0s$preAvgNTL
summary(LM6 <- lm(outcome ~ Treatment + preTrendNTL + preAvgNTL + Edu_Lvl + 
                    Religion + Household + Gender + Age + Literacy + Wealth + 
                    Marital_Status + Working + Occupation + 
                    factor(DHSREGNA),
                  data=modelDistance_ext_0s))
stargazer(LM5,LM6,
          type = "text",
          align = TRUE,
          dep.var.labels = "Night-time Lights",
          covariate.labels = c("Treatment","Night-time Lights, Pre-Trend (1992-2007)",
                               "Night-Time Lights, Pre-Average (1992-2007)","Education Level (Primary)",
                               "Education Level (Secondary)","Living in Household","Gender of Head of Household (Female)",
                               "Age of Head of Household","Literacy (High)",
                               "Wealth (Poorer)","Wealth (Middle)","Wealth (Richer)","Wealth (Richest)",
                               "Marital Status (Married)","Marital Status (Living Together)","Marital Status (Not Living Together)",
                               "Working (Yes)","Occupation (44)",
                               "DHS Region (North Central)","DHS Region (North Western)",
                               "DHS Region (South Central)","DHS Region (South Eastern A)"),
          title = "Overlap Counts (2007-2013)",
          column.labels = c("All Concessions","Extraction Concessions Only"),
          out = "0s_overlap.txt")

#third model, with 0s removed for preAvgNTL
DTA_overlaps_0s <- dhs.2007
DTA_overlaps_0s <- DTA_overlaps_0s[which(DTA_overlaps_0s@data$preAvgNTL!=0.000000e+00),]
DTA_overlaps_0s@data$Treatment <- 0
#Calculate the Median Overlap
med_overlap_0s <- median(DTA_overlaps_0s@data$overlap_counts)
#Set units with overlap >med_overlap to 1:
DTA_overlaps_0s@data$Treatment <- (as.numeric((DTA_overlaps_0s@data$overlap_counts) > 
                                                med_overlap_0s))
#Double check:
summary(DTA_overlaps_0s@data$Treatment)
#Check how many units of observation this leaves us with (our initial n for the
#analysis).
length(DTA_overlaps_0s@data)
#first stage of psm model for overlap counts, set caliper to .5 
aVars <- c("Treatment","preTrendNTL","preAvgNTL","postAvgNTL","Edu_Lvl","Religion",
           "Household","Gender","Age","Literacy","Wealth","Marital_Status",
           "Residence","Working","Occupation","DHSREGNA")
matchit.overlaps.0s <- matchit(Treatment ~ preTrendNTL + preAvgNTL + postAvgNTL + Edu_Lvl + Religion +
                                 Household + Gender + Age + Literacy + Wealth +
                                 Marital_Status + Residence + Working + Occupation + DHSREGNA, 
                               data=DTA_overlaps_0s@data[aVars],
                               method="nearest",distance="logit", 
                               caliper=0.5)
summary(matchit.overlaps.0s)
modelOverlaps_0s <- match.data(matchit.overlaps.0s)
modelOverlaps_0s$outcome <- modelOverlaps_0s$postAvgNTL - modelOverlaps_0s$preAvgNTL
summary(LM7 <- lm(outcome ~ Treatment + preTrendNTL + preAvgNTL + Edu_Lvl + 
                    Household + Gender + Age + Literacy + Wealth + 
                    Marital_Status + Working + Occupation + 
                    factor(DHSREGNA),
                  data=modelOverlaps_0s))

#fourth model, overlaps with concessions only related to extraction, 0s for preAvgNTL
DTA_overlaps_ext_0s <- dhs.2007
DTA_overlaps_ext_0s <- DTA_overlaps_ext_0s[DTA_overlaps_ext_0s@data$preAvgNTL!=0.000000e+00,]
DTA_overlaps_ext_0s@data$Treatment <- 0
#Calculate the Median Overlap
med_overlap_extractonly <- median(DTA_overlaps_ext_0s@data$overlap_counts_extractonly)
#Set units with overlap >med_overlap to 1:
DTA_overlaps_ext_0s@data$Treatment <- (as.numeric((DTA_overlaps_ext_0s@data$overlap_counts_extractonly) 
                                                  > med_overlap_extractonly))
#Double check:
summary(DTA_overlaps_ext_0s@data$Treatment)
#Check how many units of observation this leaves us with (our initial n for the
#analysis).
length(DTA_overlaps_ext_0s@data)
#first stage of psm model for overlap counts, set caliper to .5 
aVars <- c("Treatment","preTrendNTL","preAvgNTL","postAvgNTL","Edu_Lvl","Religion",
           "Household","Gender","Age","Literacy","Wealth","Marital_Status",
           "Residence","Working","Occupation","DHSREGNA")
matchit.overlaps.ext.0s <- lm(Treatment ~ preTrendNTL + preAvgNTL + postAvgNTL + Edu_Lvl + Religion +
                                  Household + Gender + Age + Literacy + Wealth +
                                  Marital_Status + Residence + Working + Occupation + DHSREGNA, 
                                data=DTA_overlaps_ext_0s@data[aVars],
                                method="nearest",distance="logit", 
                                caliper=.5)
summary(matchit.overlaps.ext.0s)
modelOverlaps_ext_0s <- match.data(matchit.overlaps.ext.0s)
modelOverlaps_ext_0s$outcome <- modelOverlaps_ext_0s$postAvgNTL - modelOverlaps_ext_0s$preAvgNTL
summary(LM8 <- lm(outcome ~ Treatment + preTrendNTL + preAvgNTL + Edu_Lvl + 
                    Household + Gender + Age + Literacy + Wealth + 
                    Marital_Status + Working + Occupation + 
                    factor(DHSREGNA),
                  data=modelOverlaps_ext_0s))
DTA_overlaps_ext_0s_outcome <- DTA_overlaps_ext_0s$postAvgNTL-DTA_overlaps_ext_0s$preAvgNTL
length(DTA_overlaps_ext_0s_outcome)
#build 1km grids for district-2007 data, and then over with dhs 2007 points
district.2007.loc <- "/home/aiddata/Desktop/Github/LiberiaInfraConflict/LISGIS_Admin Data_2007 - Copie/district-2007.shp"
dhs.2007 <- readOGR(district.2007.loc,"district-2007")